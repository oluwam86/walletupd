...
// ---------------------------------------------------------------------
// 1. Block seal: a small generative avatar derived from the address text.
// ---------------------------------------------------------------------
...
function drawBlockSeal(canvas, seed) {
  const ctx = canvas.getContext("2d");
  const size = canvas.width;
  const cell = size / 4;
  const palette = ["#F2A93B", "#C9832A", "#1B1F2A", "#EDEFF2"];
  ...
  for (let y = 0; y < 4; y++) {
    for (let x = 0; x < 2; x++) {
      const n = (seed >> ((y * 2 + x) * 3)) % palette.length;
      if (n === 0) continue;
...
function renderAccount(account, jettons, usdPerTon) {
  ...
  els.fullAddress.textContent = account.address || currentAddress;

  const balanceTon = nanoToTon(account.balance);
  els.balanceVal.textContent = `${balanceTon.toLocaleString(undefined, { maximumFractionDigits: 4 })} TON`;
  ...
  const workchain = (account.address || "").split(":")[0];
  els.chainTag.textContent = workchain !== undefined ? `Workchain ${workchain}` : "";
  els.networkLink.href = `https://tonviewer.com/${account.address || currentAddress}`;

  els.tokenCount.textContent = jettons.balances ? jettons.balances.length : 0;

  drawBlockSeal(document.getElementById("blockSeal"), hashString(account.address || currentAddress));

  document.getElementById("qrCode").innerHTML = "";
  new QRCode(document.getElementById("qrCode"), {
    text: account.address || currentAddress,
    width: 132,
    height: 132,
    colorDark: "#12151B",
    colorLight: "#ffffff",
